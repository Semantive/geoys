 ...
 */
object ADM1 extends FeatureTable[(Option[Int], String, String, Point, Int, Long, Int)]("ADM1") {
  /** Id of the country ADM belongs to. */
  def countryId   = column[Int]("country_id")

  /** Default projection for SELECT * queries. */
  def * = id.? ~ name ~ asciiName ~ location ~ countryId ~ population ~ geoId
  /** Projection for INSERT queries. */
  def forInsert = name ~ asciiName ~ location ~ countryId ~ population ~ geoId
