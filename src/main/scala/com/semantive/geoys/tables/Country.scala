 ...
 */
object Country extends FeatureTable[(Option[Int], String, String, String, String, Point, Int, Long, Int)]("country") {
  ...
  def * = id.? ~ iso2 ~ iso3 ~ name ~ asciiName ~ location ~ continentId ~ population ~ geoId
  /** Projection for INSERT queries. */
  def forInsert = iso2 ~ iso3 ~ name ~ asciiName ~ location ~ continentId ~ population ~ geoId
