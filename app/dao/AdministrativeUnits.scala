...
import models.AdministrativeUnit

object AdministrativeUnits extends Table[AdministrativeUnit]("adm4") with DAO[AdministrativeUnit] {
  ...
  def fkAdm1 = foreignKey("fk_adm4_adm1", adm1Id, AdministrativeUnits)(_.id)

  /** REFERENCES key on adm1.id. */
  def fkAdm2 = foreignKey("fk_adm4_adm2", adm2Id, AdministrativeUnits)(_.id)

  /** REFERENCES key on adm1.id. */
  def fkAdm3 = foreignKey("fk_adm4_adm3", adm3Id, AdministrativeUnits)(_.id)

  /** REFERENCES key on timezone.id. */
  def fkTimezone = foreignKey("fk_adm4_timezone", timezoneId, Timezones)(_.id)

  /** UNIQUE index on geoId. */
  def idxGeoId = index("uq_adm4_geo_id", (geoId, level), unique = true)

  /** Default projection.. */
  def * = id.? ~ name ~ asciiName ~ admCode ~ timezoneId.? ~ location.? ~ countryId ~ population.? ~ geoId ~ adm1Id.? ~ adm2Id.? ~ adm3Id.? ~ level <>(AdministrativeUnit.apply _, AdministrativeUnit.unapply _)
}