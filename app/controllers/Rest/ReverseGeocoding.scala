...
object ReverseGeocoding extends Controller {
   ...
   * @return          JSON
   */
  def findNearby(longitude: Double, latitude: Double, language: String, radius: Double, limit: Int, featureClass: Option[String], featureCode: Option[String], countryBias: Option[String]) = DBAction { implicit rs =>

    val features = Features.getByPoint(longitude, latitude, language, radius, limit, featureClass, featureCode, countryBias)
